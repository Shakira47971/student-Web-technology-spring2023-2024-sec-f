...
function namecheck($firstName, $lastName) {
    ...
    $names = array_merge(explode(" ", trim($firstName)), [$lastName]); 
...
function DOBCheck($dd,$mm,$yyyy){
    if (empty($dd) || empty($mm) || empty($yyyy)) {
        ...
        if (!is_numeric($dd) || !is_numeric($mm) || !is_numeric($yyyy)) {
            echo " Please enter numbers for day, month, and year.";
        } else {
            $dd = intval($dd);
            $mm = intval($mm);
            $yyyy = intval($yyyy);
...
if(namecheck($firstName,$lastName) && emailcheck($email)&& DOBCheck($dd,$mm,$yyyy)&& gendercheck($gender)){
    echo"Success";
}else{
    echo"";
}
